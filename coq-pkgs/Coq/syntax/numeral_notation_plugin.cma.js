...
F="(",D="after",l=")",C="Instead of Decimal.int, the types Decimal.uint or Z or Int63.int or Decimal.decimal could be used (you may need to require BinNums or Decimal or Int63 first).",h=aW.jsoo_runtime,c=h.caml_new_string,r=h.caml_register_global,aV=h.caml_wrap_exception;function
...
e=h.caml_get_global_data(),al=c("num.int63.type"),ae=c("num.int.type"),af=c("num.uint.type"),ag=c("num.decimal.type"),ab=c("num.Z.type"),ac=c("num.pos.type"),v=c("numeral_notation_plugin"),k=e.Constrexpr_ops,E=e.Global,G=e.Smartlocate,x=e.Nametab,b=e.Pp,q=e.Libnames,am=e.CErrors,n=e.Coqlib,t=e.Names,aa=e.Util,o=e.Vernacextend,B=e.Attributes,g=e.CLexer,y=e.Pcoq,i=e.Genarg,j=e.Stdarg,bD=e.CAst,bz=e.Evd,bF=e.Notation,T=e.Constrintern,S=e.Pretype_errors,R=e.Not_found,L=e.Termops,Q=e.CWarnings,aG=e.Locality,U=e.Mltop;r(43,[0,0,0],"Numeral_notation_plugin");var
bC=[0,0],bI=[0,0,1],bJ=[0,0,0],bG=[0,0,1],bH=[0,0,0],bj=c(C),bm=c(" to Decimal.int or (option Decimal.int)."),bp=c(" should go from "),a2=c(C),a5=c(")."),a8=c(" or (option "),a$=c(" should go from Decimal.int to "),aT=c("core.option.type"),I=c("option type."),J=c(") targets an "),M=c("the parsing function ("),N=c("The 'abstract after' directive has no effect when "),O=c("numbers"),P=c("abstract-large-number-no-op"),V=c(l),W=c("(warning after "),X=c(l),Y=c("(abstract after "),ad=c(l),aj=c(D),an=c("warning"),ap=c(F),as=c(l),av=c(D),ax=c("abstract"),az=c(F),aC=c("numnotoption"),aJ=c(":"),aN=c("Notation"),aP=c("Numeral"),aU=c("NumeralNotation");function
H(c){var
...
h=1;return h}catch(a){a=aV(a);if(a[1]===S[1])return 0;throw a}}function
u(by,o,j,i,bx,F){var
...
var
t=0;var
H=a(G[4],o),bA=d(G[3],0,j),bB=d(G[3],0,i),g=a(k[10],o);function
h(c,b){var
e=[0,[0,d(bD[1],0,0),0],bC,c,b];return a(k[14],e)}function
l(d){var
b=m(aT),c=[0,a(k[10],b),[0,d,0]];return a(k[15],c)}var
aP=a(E[32],H)[2][4];function
...
ar=[0,[0,y],1],N=1;else{if(f(c,e,j,h(aq,l(g))))var
aG=[0,[0,y],0],O=1;else{if(f(c,e,j,h(ap,g)))var
aH=[0,[1,y[1]],1],P=1;else{if(f(c,e,j,h(ap,l(g))))var
...
aw=[0,[0,z],1],V=1;else{if(f(c,e,i,h(g,l(av))))var
ay=[0,[0,z],0],W=1;else{if(f(c,e,i,h(g,au)))var
az=[0,[1,z[1]],1],X=1;else{if(f(c,e,i,h(g,l(au))))var
...
ax=[0,v,bA,A,bB,o,F],bK=typeof
F==="number"?0:1===F[0]?0===v[2]?(d(aO,0,ax[2]),1):0:0,bE=[0,by,bx,[1,ax],[0,a(x[41],[2,H]),0],aS,1];return a(bF[23],bE)}r(62,[0,u],"Numeral_notation_plugin__Numeral");a(U[9],v);function
w(c){if(typeof
...
aH(j,i,h,g,f,e,c){var
k=d(B[1],B[8],e),b=a(t[1][8],g);u(a(aG[7],k),j,i,h,b,f);return c}var
